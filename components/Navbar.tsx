...
type Props = {
  ...
  /** When true, prepends a back button (defaults to "← รายวิชา" → /courses) */
  showBackToCourses?: boolean;
  /** Override the back button target (default "/courses") */
  backHref?: string;
  /** Override the back button label (default "รายวิชา") */
  backLabel?: string;
  /** When true, hides the logo + Courses/Libraries switcher (e.g. hub pages where the sidebar owns navigation) */
  hideModeNav?: boolean;
};
...
export function Navbar({ brand, left, right, showBackToCourses = false, backHref, backLabel, hideModeNav = false }: Props) {
  ...
  return (
    <header className="flex shrink-0 flex-wrap items-center gap-2 border-b bg-header px-4 py-2.5 shadow-base">
      {!hideModeNav && (
        <>
          {/* Logo — clickable, returns to courses */}
          <button
            type="button"
            onClick={goHome}
            title="หน้าแรก"
      ...
      {showBackToCourses && (
        <Button
          variant="ghost"
          size="sm"
          onClick={() => router.push(backHref ?? "/courses")}
          className="!gap-1 !px-2 text-default-500 hover:text-primary"
          title={`กลับไปยัง${backLabel ?? "รายวิชา"}ทั้งหมด`}
        >
          <ArrowLeft size={14} /> {backLabel ?? "รายวิชา"}
        </Button>
      )}

      {brand && (
        <>
          {!hideModeNav && <Separator orientation="vertical" className="h-6" />}
          {brand}
        </>
      )}
